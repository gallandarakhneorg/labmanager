...
public abstract class AbstractIncomingInvitationEditor extends AbstractEntityEditor<PersonInvitation> {
	 ...
	 */
	public AbstractIncomingInvitationEditor(EditingContext context, AuthenticatedUser authenticatedUser,
	...
	public void notifySaved() {
		notifySaved(getTranslation("views.incoming_invitation.save_success", //$NON-NLS-1$
				getEditedEntity().getTitle()));
	}

	@Override
	public void notifyValidated() {
		notifyValidated(getTranslation("views.incoming_invitation.validation_success", //$NON-NLS-1$
				getEditedEntity().getTitle()));
	}

	@Override
	public void notifySavingError(Throwable error) {
		notifySavingError(error, getTranslation("views.incoming_invitation.save_error", //$NON-NLS-1$ 
				getEditedEntity().getTitle(), error.getLocalizedMessage()));
	}

	@Override
	public void notifyValidationError(Throwable error) {
		notifyValidationError(error, getTranslation("views.incoming_invitation.validation_error", //$NON-NLS-1$ 
				getEditedEntity().getTitle(), error.getLocalizedMessage()));
	}
