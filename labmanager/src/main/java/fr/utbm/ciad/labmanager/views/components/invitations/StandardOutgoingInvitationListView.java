...
public class StandardOutgoingInvitationListView extends AbstractEntityListView<PersonInvitation> {
	...
	protected Filters<PersonInvitation> createFilters() {
		return new OutgoingInvitationFilters(this::refreshGrid);
	}
	...
	protected void deleteWithQuery(Set<PersonInvitation> axes) {
		if (!axes.isEmpty()) {
			...
			ComponentFactory.createDeletionDialog(this,
					getTranslation("views.outgoing_invitation.delete.title", Integer.valueOf(size)), //$NON-NLS-1$
					getTranslation("views.outgoing_invitation.delete.message", Integer.valueOf(size)), //$NON-NLS-1$
	...
	protected void deleteCurrentSelection() {
		try {
			...
			for (final var inv : new ArrayList<>(grd.getSelectedItems())) {
				...
				final var msg = new StringBuilder("Outgoing invitation: "); //$NON-NLS-1$
	...
	/** Notify the user that the outgoing invitations were deleted.
	 *
	 * @param size the number of deleted outgoing invitations
	 */
	protected void notifyDeleted(int size) {
		notifyDeleted(size, "views.outgoing_invitation.delete_success"); //$NON-NLS-1$
	}

	/** Notify the user that the outgoing invitations cannot be deleted.
	 */
	protected void notifyDeletionError(Throwable error) {
		notifyDeletionError(error, "views.outgoing_invitation.delete_error"); //$NON-NLS-1$
	}
	...
	protected static class OutgoingInvitationFilters extends Filters<PersonInvitation> {
		 ...
		 * @param onSearch
		 */
		public OutgoingInvitationFilters(Runnable onSearch) {
			super(onSearch);
		}
